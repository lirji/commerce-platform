...
LP02 等级权益绑定、LP03 积分及 LP04–LP11 的具体 DTO/状态/接口须在各片实现前由设计 pass 补齐本文件，不能边写调用方边猜 JSON。各片只进入已落盘正式契约的范围。总体目标不因分片推迟而删减。
...
细契约见 [LP08_CONTRACT.md](LP08_CONTRACT.md)，包含扫描恢复、会员偏好、受控券节点、两类效果口径及新旧执行器发布条件。

## LP09 商品经营资料

细契约见 [LP09_CONTRACT.md](LP09_CONTRACT.md)：类目、规格模板、条码、展示资料与销售/经营检索。
