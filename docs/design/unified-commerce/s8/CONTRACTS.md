...
- Campaign.Policy新增可选terms:{percentageBps,platformFundingBps,budget}，均绑定不可变活动版本。percentageBps=0固定减免，1..10000表示原价的优惠比例，discountAmount为最大减免封顶，按分向下取整；platformFundingBps=0..10000表示活动优惠中平台承担比例，剩余商家承担；budget为该版本人民币预算上限，null表示未设上限。旧无terms默认固定减免、商家承担、无预算。
- 百分比资格有效但计算优惠不足1分时不选中；仍可解释为ELIGIBLE而实际节省0，不虚构1分优惠。保留单项活动最大优惠、同额按ID稳定择优；券叠加沿S8b1政策。
- Coupon.Definition新增可选platformFundingBps，缺省0兼容旧定义；券发行配额与活动预算独立，不能把券名义优惠算入活动预算。
- 行金额先分摊活动，再按活动后的应付金额分摊券；各资方份额按对应优惠行占比精确分配，逐行平台+商家=优惠，整单活动+券=总优惠。Quote追加资金分摊快照，已存在报价不重算。
- 活动预算数据库cap/held/spent，订单同事务条件预占held+discount≤cap-spent；不足则整单回滚。支付UNKNOWN保留，确认后held→spent，未支付取消返还held。已支付退款不恢复营销消耗预算（防退款循环重用额度），但保留退款事实供核算；未来变更政策需新版本。
- GET /v1/admin/campaign-budgets展示当前租户各预算及有界游标；订单促销预占台账保持原资方快照，不依赖后续活动暂停/新版本。
