...
| S10 | 管理台和消费端真实接口、数据库 seed、部署文件与全链路验证 | 各后端 API 完成时逐页跟进，最终依赖 S9 | frontend/runtime | 沿用已有设计资产，另出前端架构 | S10a DONE (144 backend + 4 browser); S10b TODO |
...
S7a已实现履约/部分退货/退款资金闭环（105测试），S7b权益冲正随S8台账实现，S7父项仍需二者均验收。
