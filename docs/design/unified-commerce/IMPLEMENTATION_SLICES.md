...
用户已授权新建与开始实施。首个执行视野 M0 是领域内核工程基线；其后是实际数据库/API 的 M1。当前S0–S10本地实现已完成，远程CI通过并已合并推送main，精确代码提交和CI证据见PROGRESS_STATE；外部联调按用户要求后置。下表保留分阶段验收证据。
...
| S9 | 旅程实例、等待/触达/超时/取消、恢复和运营低代码发布 | S8 | marketing/journey/lowcode | 持久调度、渠道隔离 | DONE (143 tests; S9a restart + S9b governance) |
| S10 | 管理台和消费端真实接口、数据库 seed、部署文件与全链路验证 | 各后端 API 完成时逐页跟进，最终依赖 S9 | frontend/runtime | 沿用已有设计资产，另出前端架构 | DONE（本地及远程145 backend + 4 browser；seed/容器重启通过；CI PASS） |
...
2026-09-23细化：S5a订单/库存/同事务事件先行，S5b券权益占用随S8契约实现；S6依赖S5a。S5整体须S5a与S5b均验收才DONE，未删减权益范围。详见s5/CONTRACTS.md。

S7a已实现履约/部分退货/退款资金闭环（105测试），S7b权益冲正随S8台账实现，S7父项已在S8b2b同时验收。
