...
test("生命周期：生日模板、审批、持久扫描、旅程券到账与效果比较",async({page,browser})=>{
 await login(page,true);await navigate(page, "营销旅程");
 ...
 const member=await browser.newPage();await login(member,false);await navigate(member, "优惠券");await member.getByRole("tab",{name:"我的优惠券",exact:true}).click();await expect(member.getByText("会员关怀受控券",{exact:true})).toBeVisible();
 await navigate(page, "营销效果");await page.getByRole("tab",{name:"会员营销比较",exact:true}).click();
