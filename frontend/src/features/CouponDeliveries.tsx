...
import { ActionButton, CommandModal, ErrorNotice, ListPanel, PageHead, PrimaryCell, Status, Workbench, instant, time } from "../shared/ui";
...
export function CouponDeliveries({ store }: { store: string }) {
  ...
  return <Workbench>
    <PageHead eyebrow="营销与旅程" title="定向发券" description="固定人群版本，分批发放、会员频控和可恢复进度。" extra={<Space wrap><Button onClick={refresh}>刷新批次</Button><ActionButton label="推进一批发券" path="/admin/coupon-deliveries/pump" onDone={refresh} /></Space>} />
    ...
    <ListPanel
      toolbar={<CommandModal title="创建定向发券" path="/admin/coupon-deliveries" disabled={!store} fields={[
    ...
    >
      <Table<Batch> rowKey={r => r.content.batchId} dataSource={batches.data} loading={batches.loading} pagination={false} scroll={{ x: 1250 }} columns={[
        { title: "批次", render: (_, r) => <PrimaryCell title={r.content.name} subtitle={r.content.batchId} /> },
    ...
    <Drawer className="record-drawer" title={`${selected?.content.name ?? "定向券"} · 收件人回执`} open={!!selected} onClose={() => setSelected(undefined)} size="large">
      <ErrorNotice error={recipients.error} />
      {selectedNow && <div className="record-hero"><div className="record-hero-main"><div><div className="page-eyebrow">发券批次</div><div className="record-hero-title">{selectedNow.content.name}</div><div className="record-hero-id">{selectedNow.content.batchId}</div></div><Status value={selectedNow.status}/></div><div className="record-metrics"><div className="record-metric"><span>成功发放</span><strong>{selectedNow.issued}</strong></div><div className="record-metric"><span>跳过</span><strong>{selectedNow.skipped}</strong></div><div className="record-metric"><span>撤销 / 保留</span><strong>{selectedNow.revoked} / {selectedNow.kept}</strong></div></div></div>}
      {selectedNow && controls(selectedNow)}
      <Button onClick={refresh} style={{ margin: "12px 0" }}>刷新回执</Button>
