...
public class OrderService implements OrderApi {
    private final OrderMapper mapper;private final Commands commands;private final QuoteApi quotes;private final InventoryApi inventory;
    ...
    public OrderService(OrderMapper mapper,Commands commands,QuoteApi quotes,InventoryApi inventory,MemberApi members,StoreApi stores,Outbox outbox,AddressCipher addresses,Clock clock) {
    ...
    public View create(Actor actor,String key,Create input) {
        ...
        return commands.run(actor,"order.create",key,input,View.class,()->{
            ...
            if(free) {lifecycle=lifecycle.apply(OrderEvent.PAYMENT_CONFIRMED);inventory.confirm(actor.tenantId(),id);}
    ...
    public View cancel(Actor actor,String key,String id) {
        ...
        return commands.run(actor,"order.cancel",key,id,View.class,()->{
            ...
            if(next.state()==OrderState.CANCELLED) inventory.release(actor.tenantId(),id);
