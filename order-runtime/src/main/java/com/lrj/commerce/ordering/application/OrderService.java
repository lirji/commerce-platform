...
public class OrderService implements OrderApi {
    private final com.lrj.commerce.benefit.api.CouponApi coupons;private final OrderMapper mapper;private final Commands commands;private final QuoteApi quotes;private final InventoryApi inventory;
    ...
    public OrderService(OrderMapper mapper,Commands commands,QuoteApi quotes,InventoryApi inventory,MemberApi members,StoreApi stores,Outbox outbox,AddressCipher addresses,Clock clock,com.lrj.commerce.benefit.api.CouponApi coupons) {this.coupons=coupons;
    ...
    public View create(Actor actor,String key,Create input) {
        ...
        return commands.run(actor,"order.create",key,input,View.class,()->{
            ...
            if(free) {lifecycle=lifecycle.apply(OrderEvent.PAYMENT_CONFIRMED);inventory.confirm(actor.tenantId(),id);coupons.confirm(actor.tenantId(),id);}
    ...
    public View cancel(Actor actor,String key,String id) {
        ...
        return commands.run(actor,"order.cancel",key,id,View.class,()->{
            ...
            if(next.state()==OrderState.CANCELLED){inventory.release(actor.tenantId(),id);coupons.release(actor.tenantId(),id);}
    ...
    public View paymentFact(String tenant,String id,String amount,boolean paid) {
        ...
        if(paid){inventory.confirm(tenant,id);coupons.confirm(tenant,id);}else{inventory.release(tenant,id);coupons.release(tenant,id);}
    ...
    public int expire(Actor actor,String key) {
        actor.requireAdmin();return commands.run(actor,"order.expire",key,"expire",Integer.class,()->{
            ...
            for(var row:rows) {
                ...
                if(next.state()==OrderState.CANCELLED){inventory.release(actor.tenantId(),row.orderId());coupons.release(actor.tenantId(),row.orderId());}
