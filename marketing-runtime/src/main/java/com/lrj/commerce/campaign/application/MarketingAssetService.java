...
public class MarketingAssetService implements MarketingAssets {
    ...
    public AudienceView createAudience(Actor actor,String key,Audience input){
        actor.requireAdmin();Inputs.require(input!=null&&input.memberIds()!=null&&input.memberIds().size()<=500,"人群单批最多500会员");Identifiers.require(input.audienceId());Inputs.require(!input.audienceId().startsWith("dyn-"),"dyn-前缀保留给动态人群任务");Inputs.text(input.name(),128);Inputs.text(input.source(),128);Inputs.require(input.version()>0&&input.watermark()!=null&&input.validUntil()!=null,"人群版本或时间缺失");
...
/** 固定版本需存在已发布头，不能暴露刷新中成员。 */
    public List<String> members(String tenant,Ref ref,String after,int limit){Identifiers.require(tenant);Inputs.require(ref!=null&&ref.version()>0,"人群引用无效");Identifiers.require(ref.id());Inputs.page(after,limit);Inputs.found(mapper.audienceFind(tenant,ref.id(),ref.version()));return mapper.audienceMembers(tenant,ref,after,limit);}
