...
public class MarketingAssetService implements MarketingAssets {
    ...
    private void fields(RuleNode node){if(node.kind().equals("COMPARE")){String type=Map.of("memberLevel","TEXT","orderAmount","DECIMAL").get(node.field());Inputs.require(type!=null&&type.equals(node.valueType()),"规则字段或类型不在可信目录");}else node.children().forEach(this::fields);}
}
